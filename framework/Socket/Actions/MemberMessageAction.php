...
class MemberMessageAction extends AbstractAction
{
    const ACTION_NAME = 'member-message-action';
    ...
    public function validateData(array $data): void
    {
        ...
        // var_dump($this->channelPersistence->getAssoc($this->fd), $this->memberAssocPersistence->getAssoc($this->fd));
        if (!$this->channelPersistence->getAssoc($this->fd)) {
            throw new InvalidArgumentException('잘못된 접근입니다. channel' . $this->fd);
        }
        ...
        if ($aCId[0] == 'bracket') {
            ...
            if ($isNext) {
                $this->chatRoomManager->addMessage([
                    ...
                    'type' => 0, // member(0),system(1),manager(2),admin(3)
                    'message' => $strMessage,
                        ...
                        'data' => [
                            'chat' => [
                                ...
                                'from' => [
                                    ...
                                    'entrant_id' => Arr::get($aTeam, 'team_id', $aMember['member_id']),
                                    'grade' => Arr::get($aTeam, 'team_grade')
                                ]
                            ]
                        ],
